...
public class Game extends JPanel {
    ...
    //每次调用prop的结果都不同，需要保留调用的结果用作判断；
        ...
        for (AbstractProp enemyProp : enemyProps){
            if (enemyProp.crash(heroAircraft) || heroAircraft.crash(enemyProp)) {
                if(enemyProp instanceof Blood){
                    heroAircraft.increaseHp(enemyProp.effect());
                }else{
                    enemyProp.effect();
                }
                enemyProp.vanish();
